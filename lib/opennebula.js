...
OpenNebula.prototype.getVMShowbacks = function(callback, filterFlag, firstMonth, firstYear, lastMonth, lastYear) {
  this.modem.call('vmpool.showback', [filterFlag || -1, firstMonth || -1, firstYear || -1, lastMonth || -1, lastYear || -1], function(err, data) {
    if (err) return callback(err);

    if (Array.isArray(data.SHOWBACK_RECORDS)) {
      callback(null, data.SHOWBACK_RECORDS);
    } else if(Array.isArray(data.SHOWBACK_RECORDS.SHOWBACK)) {
      callback(null, data.SHOWBACK_RECORDS.SHOWBACK);
    } else {
      callback(null, [data.SHOWBACK_RECORDS.SHOWBACK]);
    }
  });
